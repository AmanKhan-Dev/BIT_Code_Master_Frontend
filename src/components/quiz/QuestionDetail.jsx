...
const QuestionDetail = () => {
  ...
  useEffect(() => {
    const fetchQuestionData = async () => {
      try {
        const url = `http://35.226.248.183:8080/codingQuestions/questions/${questionSetId}/${questionNo}`;
  ...
  const fetchSavedCode = async () => {
    try {
      const url = `http://35.226.248.183:8080/code/get?student_email=${email}&question_set_id=${questionSetId}&question_no=${questionNo}`;
      const response = await axios.get(url);
  ...
  const saveResult = async (resultData) => {
    try {
      await axios.post('http://35.226.248.183:8080/api/results/saveResult', {
        questionSetId,
        questionNo,
        email,
  ...
  const handleVerifySubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const compileResponse = await axios.post('http://35.226.248.183:8080/api/compiler/compile', {
        sourceCode,
        language: language === 'c_cpp' ? 'C++' : 'C',
        userInput
      });
      ...
      const verifyResponse = await axios.post('http://35.226.248.183:8080/api/compiler/compileTests', {
        sourceCode,
        language,
        questionSetId,
        questionNo,
  ...
  const handleSaveCode = async () => {
    try {
      const saveCodeResponse = await axios.post('http://35.226.248.183:8080/code/save', {
        codeSaverId: {
          student_email: email,
