...
const Pallate = () => {
  ...
  useEffect(() => {
    const fetchSetInfo = async () => {
      try {
        const response = await axios.get(`http://10.128.0.2:8080/sets/${questionSetId}`); // Fetch set information
    ...
    const fetchQuestions = async () => {
      try {
        const response = await axios.get('http://10.128.0.2:8080/codingQuestions/questionsBySetId', {
  ...
  const checkSolvedQuestions = async (questions) => {
    ...
    for (const question of questions) {
      try {
        const response = await axios.get('http://10.128.0.2:8080/api/results/exists', {
          params: {
